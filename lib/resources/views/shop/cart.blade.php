    ...
    @if ($carrinhoItems->count() > 0)
        <table border="1">
            ...
            @foreach ($carrinhoItems as $item)
                <tr>
                    <td>
                        @if ($item->filme->imagem)
                        <img src="{{ asset('storage/' . $item->filme->imagem) }}" alt="{{ $item->filme->nome }}" width="100">
                    @else
                        Sem imagem
                    @endif
                </td>
                    <td>{{ $item->filme->nome }}</td>
                    <td>R$ {{ number_format($item->filme->preco, 2, ',', '.') }}</td>
                    <td>
                    ...
                    <td>R$ {{ number_format($item->filme->preco * $item->quantidade, 2, ',', '.') }}</td>
                    <td>
            ...
            <tr>
                ...
                <td>R$ {{ number_format($carrinhoItems->sum(function ($item) {
                    return $item->filme->preco * $item->quantidade;
                }), 2, ',', '.') }}</td>

            </tr>
        </table>

        <!-- Acessando o primeiro item do carrinho para passar o ID do filme para a rota do Checkout -->
        <td>
            <a class="botao-finalizar" href="{{ route('shop.checkoutFromCart', ['id' => $carrinhoItems[0]->filme->id]) }}">Finalizar Compra</a>
        </td>
